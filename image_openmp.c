...
//            The outer row loop is parallelized. No race conditions because:
//            - srcImage->data is read-only across all threads
//            - Each (row, pix, bit) triple maps to a unique index in destImage->data
//Parameters: srcImage: The image being convoluted
//            destImage: A pointer to a pre-allocated structure to receive the result
//            algorithm: The kernel matrix to use for the convolution
//            numThreads: Number of OpenMP threads to use
//Returns: Nothing
...
//Returns: -1
int Usage() {
    printf("Usage: ./image_openmp <filename> <type> [threads]\n");
...
int main(int argc, char** argv) {
    ...
    t2 = time(NULL);
    printf("Took %ld seconds with OpenMP using %d thread(s)\n", (long)(t2 - t1), numThreads);
    return 0;
}
